...
def linreg(darray):
    p = darray.polyfit(dim='time', deg=1)
    trend = xr.polyval(darray.time, p.polyfit_coefficients)
    return trend
...
def compute_seasonality(darray):
    seasonality = (darray
        ...
        .mean()
        .rolling(dayofyear=30, center=True, min_periods=1)
        .mean()
    )
    return seasonality
...
def compute_anomaly(darray):
    trend = linreg(darray)
    detrended = darray - trend
    detrended_anom = detrended.groupby('time.dayofyear') - compute_seasonality(darray)
    detrended_anom = detrended_anom.drop_vars('dayofyear')
    anom = detrended_anom + trend
    return anom
...
def main():
    ...
    for season in ['DJF', 'JJA']:
        ...
        for station_name, timeseries in ds.items():
            # Compute seasonal anomalies
            timeseries = compute_anomaly(timeseries)
            
            # Select season and process timeseries
            timeseries = select_season(timeseries, season)
